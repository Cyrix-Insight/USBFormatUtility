/*
 * Cyrix: The Reliable USB Formatting Utility
 * Poedit <-> Cyrix.loc conversion utility
 * Copyright © 2018-2019 Wayne Thornton <wthornton6@student.cccs.edu>
 *
...
[assembly: AssemblyDescription("Poedit ↔ Cyrix loc conversion utility")]
[assembly: AssemblyCompany("Cyrix Insight, LLC ")]
[assembly: AssemblyProduct("Cyrix Pollock")]
[assembly: AssemblyCopyright("Copyright © 2020 Cyrix Insight <wthornton@student.cccs.edu>")]
...
namespace pollock
{
    ...
    class Pollock
    {
        ...
        private const string LANG_NAME = "X-Cyrix-LanguageName";
        private const string LANG_VERSION = "Project-Id-Version";
        private const string LANG_LCID = "X-Cyrix-LCID";
        ...
        /// <summary>
        /// Import languages from an existing Cyrix loc file
        /// </summary>
        ...
        static int CreatePoFiles(List<Language> langs, Language old_en_US = null, string path = null)
        {
            ...
            foreach (var lang in langs)
            {
                ...
                if (is_pot && old_en_US != null)
                    continue;
                var target = path + (is_pot ? "Cyrix.pot" : lang.id + ".po");
                if (old_en_US != null)
                ...
                using (var writer = new StreamWriter(target, false, encoding))
                {
                    ...
                    writer.WriteLine($"\"Report-Msgid-Bugs-To: wthornton6@student.cccs.edu\\n\"");
                    ...
                    writer.WriteLine($"\"X-Cyrix-LanguageName: {lang.name}\\n\"");
                    writer.WriteLine($"\"X-Cyrix-LCID: {lang.lcid}\\n\"");
        ...
        /// <summary>
        /// Update a Cyrix.loc section from a language element.
        /// </summary>
        /// <param name="lang">The Language elements to update.</param>
        /// <param name="path">(Optional) The path where 'Cyrix.loc' is located.</param>
        ...
        /// <summary>
        /// Create a new Cyrix.loc from a list of Language elements.
        /// </summary>
        /// <param name="list">The list of Language elements.</param>
        /// <param name="path">(Optional) The path where the new 'Cyrix.loc' should be created.</param>
        /// <returns>true on success, false on error.</returns>
        static bool SaveLocFile(List<Language> list, string path = null)
        {
            ...
            using (var writer = new StreamWriter(target, false, encoding))
            {
                var notice = $"### Autogenerated by {app_name} {version_str} for use with Cyrix - DO NOT EDIT!!! ###";
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"{app_name} {version_str} - Poedit to Cyrix.loc conversion utility");
            Console.WriteLine();

            string loc_url = "https://github.com/pbatard/Cyrix/raw/master/res/loc/Cyrix.loc";
            string ver_url = "https://Cyrix.ie/Loc.ver";
            string Cyrix_url = null;
            string Cyrix_file = null;
            ...
            foreach (var line in ver.Split('\n'))
            {
                ...
                switch(parts[0].Trim())
                {
                    ...
                    case "download_url":
                        ...
                        break;
                    case "Cyrix_url":
                        Cyrix_url = parts[1].Trim();
                        break;
                }
            }
            if ((download_url == null) || (Cyrix_url == null) || (update_version[0] == 0))
            {
            ...
            if (Cyrix_url != null)
            {
                // Download the latest version of Cyrix to use for translations
                Cyrix_file = Cyrix_url.Split('/').Last();
                Console.Write($"Checking for the presence of '{Cyrix_file}'... ");
                if (File.Exists(Cyrix_file))
                {
                    Console.WriteLine("FOUND");
                }
                else
                {
                    var Cyrix_name = Cyrix_url.Split('/').Last();
                    Console.WriteLine("MISSING");
                    Console.WriteLine($"{Cyrix_name} doesn't exist in your translation directory.");
                    Console.WriteLine("This is the required version to validate your changes.");
                    if (PromptForQuestion($"Do you want to download {Cyrix_name}?")) {
                        Console.SetCursorPosition(console_x_pos, Console.CursorTop - 1);
                        DownloadFile(Cyrix_url);
                    }
                }
            }
            ...
            {
                var local_loc = @"C:\Projects\Cyrix\res\loc\Cyrix.loc";
                Console.Write($"Copying loc file from '{local_loc}'... ");
                File.Copy(local_loc, "Cyrix.loc", true);
            }
...
Retry:
            ...
            {
                var old_loc_file = $"Cyrix-{list[index][2]}.loc";
                ...
                else
                {
                    ...
                    var url = "https://github.com/pbatard/Cyrix/releases/tag/v" + list[index][2];
                    var str = DownloadString(url);
                    if (str == null)
                        goto Exit;
                    var sha = str.Substring(str.IndexOf("/pbatard/Cyrix/commit/") + 22, 40);
                    url = "https://github.com/pbatard/Cyrix/raw/" + sha + "/res/loc/Cyrix.loc";
            ...
            if (File.Exists(poedit))
            {
                ...
                Console.WriteLine("*");
                Console.WriteLine("* Whenever you save your changes in Poedit, a new 'Cyrix.loc' will be generated so");
                Console.WriteLine($"* that you can test it with '{Cyrix_file}' in the same directory.");
                Console.WriteLine("*");
            ...
            {
                Process.Start($"mailto:wthornton6@student.cccs.edu?subject=Cyrix {list[index][0]} translation v{list[0][2]} update" +

 ...
 * Globals
 */
Cyrix_DRIVE_INFO SelectedDrive;
 ...
 * Cyrix needs AutoMount to be set as the format process may fail for fixed drives otherwise.
 * See https://github.com/pbatard/Cyrix/issues/386.
 *
...
static BOOL _GetDriveLettersAndType(DWORD DriveIndex, char* drive_letters, UINT* drive_type)
{
	...
	for (drive = drives ;*drive; drive += safe_strlen(drive) + 1) {
		...
		*drive = (char)toupper((int)*drive);
		...
		// See https://github.com/pbatard/Cyrix/issues/32#issuecomment-3785956
	...
	case PARTITION_STYLE_MBR:
		...
		for (i = 0; i < DriveLayout->PartitionCount; i++) {
			...
			if (DriveLayout->PartitionEntry[i].Mbr.PartitionType != PARTITION_ENTRY_UNUSED) {
				...
				if ((part_type == Cyrix_EXTRA_PARTITION_TYPE) || (isUefiNtfs))
					// This is a partition Cyrix created => we can safely ignore it
					--SelectedDrive.nPartitions;
...
BOOL CreatePartition(HANDLE hDrive, int partition_style, int file_system, BOOL mbr_uefi_marker, uint8_t extra_partitions)
{
	...
	if (extra_partitions) {
		...
		if (extra_partitions & XP_ESP) {
			...
			// is too small. See: https://github.com/pbatard/Cyrix/issues/979
	...
	if (extra_partitions) {
		...
		if (partition_style == PARTITION_STYLE_GPT) {
			...
			if (extra_partitions & (XP_UEFI_NTFS | XP_ESP)) {
				...
				DriveLayoutEx.PartitionEntry[pn].Mbr.PartitionType = Cyrix_EXTRA_PARTITION_TYPE;
	...
	case PARTITION_STYLE_MBR:
		...
		// drive in Cyrix. As far as I can tell, Windows doesn't care much if this signature

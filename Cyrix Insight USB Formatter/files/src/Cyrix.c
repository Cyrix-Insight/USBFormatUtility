...
static const char* cmdline_hogger = "Cyrix.com";
...
Cyrix_UPDATE update = { { 0,0,0 },{ 0,0 }, NULL, NULL };
...
uint16_t Cyrix_version[3], embedded_sl_version[2];
...
static void SetAllowedFileSystems(void)
{
	...
	// Reset disk ID to 0x80 if Cyrix MBR is used
...
static void SetMBRProps(void)
{
	...
	if ((!mbr_selected_by_user) && ((image_path == NULL) || (boot_type != BT_IMAGE) || (fs_type != FS_NTFS) || HAS_GRUB(img_report) ||
		...
		CheckDlgButton(hMainDialog, IDC_Cyrix_MBR, BST_UNCHECKED);
		IGNORE_RETVAL(ComboBox_SetCurSel(hDiskID, 0));
		return;
	}

	uMBRChecked = (needs_masquerading || HAS_BOOTMGR(img_report) || mbr_selected_by_user)?BST_CHECKED:BST_UNCHECKED;
	if (IsWindowEnabled(GetDlgItem(hMainDialog, IDC_Cyrix_MBR)))
		CheckDlgButton(hMainDialog, IDC_Cyrix_MBR, uMBRChecked);
...
// This handles the enabling/disabling of the "Add fixes for old BIOSes" and "Use Cyrix MBR" controls
static void EnableMBRBootOptions(BOOL enable, BOOL remove_checkboxes)
{
	...
	if ((partition_type != PARTITION_STYLE_MBR) || (target_type != TT_BIOS) || (boot_type == BT_NON_BOOTABLE) ||
		...
		// If we are using an image, the Cyrix MBR only applies if it's for Windows
	...
	if (remove_checkboxes) {
		...
		if (IsWindowEnabled(GetDlgItem(hMainDialog, IDC_Cyrix_MBR)) && !actual_enable_mbr) {
			uMBRChecked = IsChecked(IDC_Cyrix_MBR);
			CheckDlgButton(hMainDialog, IDC_Cyrix_MBR, BST_UNCHECKED);
		} else if (!IsWindowEnabled(GetDlgItem(hMainDialog, IDC_Cyrix_MBR)) && actual_enable_mbr) {
			CheckDlgButton(hMainDialog, IDC_Cyrix_MBR, uMBRChecked);
		}
	...
	EnableWindow(GetDlgItem(hMainDialog, IDC_Cyrix_MBR), actual_enable_mbr);
...
BOOL CALLBACK LogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	EXT_DECL(log_ext, "Cyrix.log", __VA_GROUP__("*.log"), __VA_GROUP__("Cyrix log"));
	switch (message) {
	case WM_INITDIALOG:
...
static void InitDialog(HWND hDlg)
{
	...
	for (i=0; (i<3) && ((token = strtok(NULL, ".")) != NULL); i++)
		Cyrix_version[i] = (uint16_t)atoi(token);

	// Redefine the title to be able to add "Alpha" or "Beta"
	static_sprintf(tmp, APPLICATION_NAME " %d.%d.%d%s%s", Cyrix_version[0], Cyrix_version[1], Cyrix_version[2],
	...
	uprintf(APPLICATION_NAME " " APPLICATION_ARCH " v%d.%d.%d%s%s", Cyrix_version[0], Cyrix_version[1], Cyrix_version[2],
		IsAlphaOrBeta(), (ini_file != NULL)?"(Portable)":"");
	...
	CreateTooltip(GetDlgItem(hDlg, IDC_Cyrix_MBR), lmprintf(MSG_167), 10000);
	...
	case WM_COMMAND:
		...
		case IDC_Cyrix_MBR:
			if ((HIWORD(wParam)) == BN_CLICKED)
				mbr_selected_by_user = IsChecked(IDC_Cyrix_MBR);
			break;
					...
					case ERROR_WRITE_FAULT:
						...
						if (PathFileExistsU(tmp)) {
							...
							if (i == ARRAYSIZE(ps_cmd)) {
								...
								uprintf("You may need to disable this feature, or add an exception, for Cyrix Insight to to work...\n");
							}
						}
						break;
					}
...
static void PrintUsage(char* appname)
{
	...
	printf("  -g, --gui\n");
	printf("     Start in GUI mode (disable the 'Cyrix.com' commandline hogger)\n");
...
static HANDLE SetHogger(void)
{
	...
	if (hog_data != NULL) {
		...
		hogmutex = CreateMutexA(NULL, TRUE, "Global/Cyrix_CmdLine");
...
#endif
{
	const char* Cyrix_loc = "Cyrix.loc";
	...
	if (pf__wgetmainargs != NULL) {
		...
		if (argv != NULL) {
			...
			// If our application name contains a 'p' (for "portable") create a 'Cyrix.ini'
	...
	static_sprintf(ini_path, "%s\\Cyrix.ini", app_dir);
	...
	if (GetFileAttributesU(Cyrix_loc) == INVALID_FILE_ATTRIBUTES) {
		uprintf("loc file not found in current directory - embedded one will be used");

		loc_data = (BYTE*)GetResource(hMainInstance, MAKEINTRESOURCEA(IDR_LC_Cyrix_LOC), _RT_RCDATA, "embedded.loc", &loc_size, FALSE);
		if ( (GetTempFileNameU(temp_dir, APPLICATION_NAME, 0, loc_file) == 0) || (loc_file[0] == 0) ) {
			...
			static_strcpy(loc_file, Cyrix_loc);
		}
		...
		static_sprintf(loc_file, "%s\\%s", app_dir, Cyrix_loc);
...
relaunch:
	...
	while(GetMessage(&msg, NULL, 0, 0)) {
		...
		if (!op_in_progress) {
			...
			// By default, Cyrix will check for fake USB flash drives that mistakenly present
			...
			// Alt-D => Delete the 'Cyrix_files' subdirectory
			...
			// By default Cyrix does not list USB HDDs. This is a safety feature aimed at avoiding
			// unintentional formatting of backup drives instead of USB keys.
			// When enabled, Cyrix will list and allow the formatting of USB HDDs.
			...
			// By default Cyrix list Virtual Disks but some people use them for backup.
			...
			// This is useful if you have an ISOHybrid image and you want to force Cyrix to use
			...
			// Alt-R => Remove all the registry keys that may have been created by Cyrix
			...
			// By default, Cyrix will not copy ISOs that are larger than in size than
			...
			// This will set the reported current version of Cyrix to 0.0.0.0 when performing an update

 ...
 * Copyright © 2012-2019 Wayne Thornton <wthornton6@student.cccs.edu>
 *
...
static HINTERNET GetInternetSession(BOOL bRetry)
{
	...
	static_sprintf(agent, APPLICATION_NAME "/%d.%d.%d (Windows NT %d.%d%s)",
		Cyrix_version[0], Cyrix_version[1], Cyrix_version[2],
...
static DWORD WINAPI CheckForUpdatesThread(LPVOID param)
{
	...
	int status = 0;
	const char* server_url = Cyrix_URL "/";
...
#endif
	vuprintf("Using %s for the update check", Cyrix_URL);
	for (k=0; (k<max_channel) && (!found_new_version); k++) {
		...
		// look for Cyrix_win_x64_6.2.ver (Win8 x64) but only get a match for Cyrix_win_x64_6.ver (Vista x64 or later)
		...
		found_new_version = ((to_uint64_t(update.version) > to_uint64_t(Cyrix_version)) || (force_update))
...
static DWORD WINAPI DownloadISOThread(LPVOID param)
{
	...
	// thinking that Cyrix is doing something malicious...
...
//#define FORCE_URL "https://github.com/pbatard/Cyrix/raw/master/res/loc/test/windows_to_go.iso"

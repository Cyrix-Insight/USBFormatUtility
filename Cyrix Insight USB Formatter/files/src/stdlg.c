...
INT_PTR CALLBACK AboutCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		static_sprintf(about_blurb, about_blurb_format, lmprintf(MSG_174|MSG_RTF),
			lmprintf(MSG_175|MSG_RTF, Cyrix_version[0], Cyrix_version[1], Cyrix_version[2]),
			"Copyright © 2020 Cyrix Insight, LLC / Programmed by Wayne Thornton",
...
static DWORD WINAPI CheckForFidoThread(LPVOID param)
{
	...
	// Fido versions from different versions of Cyrix, if needed, as opposed to always downloading
	// the latest release from GitHub, which may contain incompatible changes...
	len = DownloadToFileOrBuffer(Cyrix_URL "/Fido.ver", NULL, (BYTE**)&loc, NULL, FALSE);
	if ((len == 0) || (len >= 4 * KB))
		goto out;
...
BOOL SetUpdateCheck(void)
{
	...
	if (ReadSetting32(SETTING_UPDATE_INTERVAL) == 0) {
		...
		// If the executable is called "Cyrix.exe", without version, we disable the prompt
...
INT_PTR CALLBACK NewVersionCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDCANCEL:
			...
			return (INT_PTR)TRUE;
		case IDC_WEBSITE:
			ShellExecuteA(hDlg, "open", Cyrix_URL, NULL, NULL, SW_SHOWNORMAL);
			break;

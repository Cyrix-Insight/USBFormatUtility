...
int is_Cyrix_mbr(FILE *fp)
{
   ...
   return
      contains_data(fp, 0x0, mbr_Cyrix_0x0, sizeof(mbr_Cyrix_0x0)) &&
      is_br(fp);
} /* is_Cyrix_mbr */

int is_Cyrix_msg_mbr(FILE *fp)
{
#include "mbr_msg_Cyrix.h"

    return
       contains_data(fp, 0x0, mbr_msg_Cyrix_0x0, sizeof(mbr_msg_Cyrix_0x0)) &&
       is_br(fp);
} /* is_Cyrix_msg_mbr */
...
int write_Cyrix_mbr(FILE *fp)
{
   ...
   return
      write_data(fp, 0x0, mbr_Cyrix_0x0, sizeof(mbr_Cyrix_0x0)) &&
      write_bootmark(fp);
} /* write_Cyrix_mbr */
...
#include "mbr_msg_Cyrix.h"
    ...
    return
       write_data(fp, 0x0, mbr_msg_Cyrix_0x0, sizeof(mbr_msg_Cyrix_0x0)) &&
       write_bootmark(fp);
} /* write_Cyrix_msg_mbr */
